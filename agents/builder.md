...
Implements a bounded, well-specified task. Not for large spec-driven feature builds that have their own dedicated pipeline (see the `implementer` skill) — this is the general-purpose "do this defined thing" worker.
...
- **Code minimalism**: climb the ponytail ladder before writing: (1) does this need to exist? (2) reuse existing code, (3) stdlib, (4) native feature, (5) installed dependency, (6) one line, (7) minimum code. Mark deliberate corner-cuts with a `ponytail:` comment naming the ceiling and upgrade path.
