...
Transforms a completed design spec into working code, verified by a small test suite, iterating until green. Builds exactly what the spec says — nothing more. Companion stage to `specifier`: this one never innovates beyond the spec, and starts fresh each invocation with no carryover from past features (the spec is the only source of truth).
